...
import { filterChangeInput,filterChangeCheckBox,filterPriorityChange } from '../../redux/actions';
const { Search } = Input;
...
export default function Filters() {
  ...
  const handleChangeInput = (e) =>{
    ...
    dispatch(filterChangeInput(e.target.value))
  }

  const handleStatusChange = (e) => {
    setStatusFilterChange(e.target.value)
    dispatch(filterChangeCheckBox(e.target.value))
  }

  const handlePriorityChange = (value) => {
    setPriorityChange(value)
    dispatch(filterPriorityChange(value))
  }
  return (
    <Row justify='center'>
      <Col span={24}>
